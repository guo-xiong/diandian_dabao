...
newFile = path + "\\"+FCT_name+"_Applovin.html"
copy(indexFile,newFile)
with open("FCT(2d)\\FCT_link.txt",'r',encoding='utf-8') as file:
    FCT_link = file.read()
    replaceFile("FCT(2d)\\log.txt",newFile,FCT_link)

print("Facebook")
#Facebook
newFile = path + "\\"+FCT_name+"_Facebook\\index.html"
copy(indexFile,newFile)
copy(res,path + "\\"+FCT_name+"_Facebook\\res.js")
#删除第一个script
...
with open(newFile, 'w',encoding='utf-8') as file:
    file.write(filedata)
with open("FCT(2d)\\Facebook_tz.txt",'r',encoding='utf-8') as file:
    new = file.read()
    replaceFile("FCT(2d)\\log.txt",newFile,new)
with open("FCT(2d)\\Facebook_head_new.txt",'r',encoding='utf-8') as file:
    new = file.read()
    replaceFile("FCT(2d)\\Facebook_head.txt",newFile,new)
    zip_file_path = path + "\\"+FCT_name+"_Facebook.zip"

create_zip_with_files(zip_file_path,[path + "\\"+FCT_name+"_Facebook\\index.html",path + "\\"+FCT_name+"_Facebook\\res.js"])


print("Google")
#Google
newFile = path + "\\"+FCT_name+"_Google\\index.html"
copy(indexFile,newFile)
with open("FCT(2d)\\Google_tz.txt",'r',encoding='utf-8') as file:
    new = file.read()
    replaceFile("FCT(2d)\\log.txt",newFile,new)

with open("FCT(2d)\\Google_head_new.txt",'r',encoding='utf-8') as file:
    new = file.read()
    replaceFile("FCT(2d)\\Google_head.txt",newFile,new)
    zip_file_path = path + "\\"+FCT_name+"_Google.zip"
